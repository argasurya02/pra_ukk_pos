...
$user = userLogin();
if ($user && isset($user['level']) && $user['level'] == 3) {
...
function insert($data){
    ...
    if(mysqli_num_rows($cekBarcode) > 0){
        echo "<script>
                    alert('Barcode sudah terpakai, barang gagal ditambahkan');
                </script>";
        return false;
    }
...
function update($data)
{
    ...
    if ($barcode !== $curBarcode) {
        if (mysqli_num_rows($cekBarcode) > 0) {
            echo "<script>
                        alert('Barcode sudah terpakai, barang gagal diupdate');
                    </script>";
            return false;
        }
    }
    ...
    if (!empty($gambar)) {
        $url = "index.php";
        $nmgbr = $id; // Nama gambar akan selalu sesuai ID

        // Jika gambar lama bukan default, hapus gambar lama
        if ($gbrLama != 'deafult_brg.png') {
            unlink('../asset/image/' . $gbrLama);
        }

        // Upload gambar baru dengan nama sesuai ID
    ...
    // Update database
    mysqli_query($koneksi, "UPDATE tbl_barang SET barcode = '$barcode', nama_barang = '$name', harga_beli = $harga_beli, harga_jual = $harga_jual, satuan = '$satuan', stock_minimal = $stockmin, gambar = '$imgBrg' WHERE id_barang = '$id' ");
