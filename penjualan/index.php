...
<div class="content-wrapper">
    ...
    <section>
        <div class="container-fluid">
            <form action="" method="post">
                        ...
                        <tbody>
                            ...
                            foreach ($brgDetail as $detail) { ?>
                                <tr>
                                    ...
                                    <td class="text-center"><a href="?barcode=<?= $detail['barcode'] ?>&idjual=<?= $detail['no_jual'] ?>&qty=<?= $detail['qty'] ?>&tgl=<?= $detail['tgl_jual'] ?>&msg=deleted" class="btn btn-sm btn-danger" title="hapus barang" onclick="return confirm('Apakah anda yakin ingin menghapus data ini?')"><i class="fas fa-trash"></i></a></td>
                                </tr>
                            <?php } ?>
                        </tbody>
                    </table>
                ...
                <div class="row">
                    <!-- Only showing the modified part of the form for customer selection -->
                    <div class="col-lg-4 p-2">
                        <div class="form-group row mb-2">
                            <label for="costumer" class="col-sm-3 col-form-label col-form-label-sm">Customer</label>
                            <div class="col-sm-9">
                                <select name="id_pelanggan" id="id_pelanggan" class="form-control form-control-sm">
                                    <?php
                                    $pelanggan = getdata("SELECT * FROM tbl_pelanggan ORDER BY nama DESC");
                                    foreach ($pelanggan as $plg) { ?>
                                        <option value="<?= $plg['id_pelanggan'] ?>" data-nama="<?= $plg['nama'] ?>"><?= $plg['nama']  ?></option>
                                    <?php }
                                    ?>
                                </select>
                                <input type="hidden" name="costumer" id="costumer">
                            </div>
                        </div>
                        <div class="form-group row mb-2">
    ...
    <script>
        ...
        let costumer = document.getElementById('costumer');

        // Set initial value
        if (idPelanggan.options.length > 0) {
            costumer.value = idPelanggan.options[idPelanggan.selectedIndex].dataset.nama;
        }

        // Update when selection changes
        idPelanggan.addEventListener('change', function() {
            costumer.value = idPelanggan.options[idPelanggan.selectedIndex].dataset.nama;
        });
    </script>
